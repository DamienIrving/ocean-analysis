...
| GISS-E2-1-G | [Website](https://data.giss.nasa.gov/modelE/cmip6/), Reference, ES-DOC | | | 1. `wfo` wrong sign <br> 2. surface ocean variables (e.g. `tos`, `hfds`) on different grid to full depth ocean (`thetao`, `so`). I've created `gn-surface` basin and areacello files to handle this. |
