...
| CESM2 (CESM2-FV2, CESM2-FV2-WACCM, CESM2-WACCM) | Website, Reference, [ES-DOC](https://explore.es-doc.org/cmip6/models/ncar/cesm2) |  |  | 1. Bogus global `volcello` (after adding missing land mask) and `areacello` totals - use `gr` and calculate own `Ofx` variables. <br> 2. Missing history attribute in `thetao` (and probably other) files. |
