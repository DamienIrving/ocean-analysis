...
function usage {
    echo "USAGE: bash $0 area_file basin_file temperature_files"
...
for tfile in "${tfiles[@]}"; do
    ...
    outfile=`echo ${outfile} | sed s:thetao:water-mass:`
    outfile=`echo ${outfile} | sed s:thetao:surface-water-mass:`

    sfile=`echo ${tfile} | sed s:thetao:so:g`

    command="${python} ${script_dir}/calc_water_mass_components.py ${afile} ${bfile} ${outfile} --salinity_files ${sfile} --temperature_files ${tfile}"
