...
if [ "${data_var}" == "temp_nonlocal_KPP" ] ; then
    long_name='"cp*rho*dzt*nonlocal tendency from KPP"'
elif [ "${data_var}" == "temp_vdiffuse_diff_cbt" ] ; then
    long_name='"vert diffusion of heat due to diff_cbt"'
elif [ "${data_var}" == "mixdownslope_temp" ] ; then
    long_name='"cp*mixdownslope*rho*dzt*temp"'
elif [ "${data_var}" == "temp_sigma_diff" ] ; then
    long_name='"thk wghtd sigma-diffusion heating"'
elif [ "${data_var}" == "temp_vdiffuse_k33" ] ; then
    long_name='"vert diffusion of heat due to K33 from neutral diffusion"'
elif [ "${data_var}" == "neutral_diffusion_temp" ] ; then
    long_name='"rho*dzt*cp*explicit neutral diffusion tendency (heating)"'
elif [ "${data_var}" == "temp_tendency" ] ; then
