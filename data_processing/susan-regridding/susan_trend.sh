...
coefficient_command="${python} ${script_dir}/calc_drift_coefficients.py ${r87_dir}/piControl/yr/ocean/r1i1p1/${variable}/latest/${variable}_Oyr_${model}_piControl_r1i1p1_??????-??????_susan-grid.nc sea_water_potential_temperature ${coef_file} --remove_outliers"
...
dedrift_command="${python} ${script_dir}/remove_drift.py ${data_dir}/${variable}_Oyr_${model}_${experiment}_${rip}_??????-??????_susan-grid.nc sea_water_potential_temperature annual ${coef_file} ${data_dir}/dedrifted/ --branch_time 342005"
