...
if [ "${var}" == "hfds" ] && [ "${inferred}" == true ] ; then
    indtype=${var}-inferred-sum-hemispheric-metrics
else
    indtype=${var}-sum-hemispheric-metrics
fi
...
var_long=${long_name}_${region}_sum
...
experiment_file=${experiment_dir}/${indtype}_${tscale}_${model}_historical-${experiment}_${rip}_${tdetails}.nc
dedrifted_dir=${experiment_dir}/dedrifted
dedrifted_file=${dedrifted_dir}/${outdtype}_${tscale}_${model}_historical-${experiment}_${rip}_${tdetails}.nc
