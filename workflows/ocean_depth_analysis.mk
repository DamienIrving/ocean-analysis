...
VOLCELLO_SUM_DIR=${MY_DATA_DIR}/${INSTITUTION}/${MODEL}/${FX_EXP}/${RUN}/Ofx/volcello/${GRID}/${VOLCELLO_VERSION}
VOLCELLO_SUM_FILE=${VOLCELLO_SUM_DIR}/volcello-basin-sum_Ofx_${MODEL}_${FX_EXP}_${RUN}_${GRID}.nc
${VOLCELLO_SUM_FILE} : ${VOLCELLO_FILE} ${BASIN_FILE}
	mkdir -p ${VOLCELLO_SUM_DIR}
	${PYTHON} ${DATA_SCRIPT_DIR}/calc_basin_aggregate.py $< ocean_volume $(word 2,$^) $@

# thetao(year, depth, basin)

TEMPERATURE_MEAN_DIR_HIST=${MY_DATA_DIR}/${INSTITUTION}/${MODEL}/historical/${RUN}/Oyr/thetao/${GRID}/${HIST_VERSION}
TEMPERATURE_MEAN_FILE_HIST=${TEMPERATURE_MEAN_DIR_HIST}/thetao-basin-mean_Oyr_${MODEL}_historical_${RUN}_${GRID}_${HIST_TIME}.nc
${TEMPERATURE_MEAN_FILE_HIST} : ${BASIN_FILE} ${VOLCELLO_FILE}
	mkdir -p ${TEMPERATURE_MEAN_DIR_HIST}
	${PYTHON} ${DATA_SCRIPT_DIR}/calc_basin_aggregate.py ${TEMPERATURE_FILES_HIST} sea_water_potential_temperature mean $< $@ --weights $(word 2,$^) --annual

TEMPERATURE_MEAN_DIR_CNTRL=${MY_DATA_DIR}/${INSTITUTION}/${MODEL}/piControl/${RUN}/Oyr/thetao/${GRID}/${CNTRL_VERSION}
TEMPERATURE_MEAN_FILE_CNTRL=${TEMPERATURE_MEAN_DIR_CNTRL}/thetao-basin-mean_Oyr_${MODEL}_piControl_${RUN}_${GRID}_${CNTRL_TIME}.nc
${TEMPERATURE_MEAN_FILE_CNTRL} : ${BASIN_FILE} ${VOLCELLO_FILE}
	mkdir -p ${TEMPERATURE_MEAN_DIR_CNTRL}
	${PYTHON} ${DATA_SCRIPT_DIR}/calc_basin_aggregate.py ${TEMPERATURE_FILES_CNTRL} sea_water_potential_temperature mean $< $@ --weights $(word 2,$^) --annual

# remove drift

DRIFT_COEFFICIENT_FILE=${TEMPERATURE_MEAN_DIR_CNTRL}/thetao-basin-mean-coefficients_Oyr_${MODEL}_piControl_${RUN}_${GRID}_${CNTRL_TIME}.nc
...
${TEMPERATURE_MEAN_DEDRIFTED_FILE} : ${TEMPERATURE_MEAN_FILE_HIST} ${DRIFT_COEFFICIENT_FILE}
	${PYTHON} ${DATA_SCRIPT_DIR}/remove_drift.py $< sea_water_potential_temperature $(word 2,$^) $@ ${BRANCH_YEAR}
...
${TEMPERATURE_MEAN_PLOT_FILE} : ${DRIFT_COEFFICIENT_FILE} ${TEMPERATURE_MEAN_FILE_CNTRL} ${TEMPERATURE_MEAN_FILE_HIST} ${TEMPERATURE_MEAN_DEDRIFTED_FILE}
	${PYTHON} ${VIZ_SCRIPT_DIR}/plot_drift.py sea_water_potential_temperature $@ --coefficient_file $< --control_files $(word 2,$^) --experiment_files $(word 3,$^) --dedrifted_files $(word 4,$^) --grid_point 20 -1 ${BRANCH_YEAR}



# targets

all : ${TEMPERATURE_MEAN_PLOT_FILE}
