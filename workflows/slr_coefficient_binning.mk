...
TEMPERATURE_FILES_EXP := $(sort $(wildcard ${CMIP6_DATA_DIR}/${PROJECT}/${MIP}/${INSTITUTION}/${MODEL}/${EXPERIMENT}/${EXP_RUN}/Omon/bigthetao/${GRID_OCEAN}/${EXP_VERSION}/bigthetao*.nc))
...
TEMPERATURE_FILES_CNTRL := $(sort $(wildcard ${CMIP6_DATA_DIR}/${PROJECT}/CMIP/${INSTITUTION}/${MODEL}/piControl/${CNTRL_RUN}/Omon/bigthetao/${GRID_OCEAN}/${CNTRL_VERSION}/bigthetao*${CNTRL_FILE_END}))
...
${BINNED_BETA_FILE_EXP} : ${BASIN_FILE} ${VOLCELLO_FILE}
	...
	${PYTHON} ${DATA_SCRIPT_DIR}/water_mass_binning.py ${BETA_FILES_EXP} saline_contraction_coefficient $< ${BIN_FREQ} $@ --salinity_files ${SALINITY_FILES_EXP} --salinity_var sea_water_salinity --temperature_files ${TEMPERATURE_FILES_EXP} --temperature_var sea_water_conservative_temperature --volume_file $(word 2,$^)
...
${BINNED_BETA_FILE_CNTRL} : ${BASIN_FILE} ${VOLCELLO_FILE}
	...
	${PYTHON} ${DATA_SCRIPT_DIR}/water_mass_binning.py ${BETA_FILES_CNTRL} saline_contraction_coefficient $< ${BIN_FREQ} $@ --salinity_files ${SALINITY_FILES_CNTRL} --salinity_var sea_water_salinity --temperature_files ${TEMPERATURE_FILES_CNTRL} --temperature_var sea_water_conservative_temperature --volume_file $(word 2,$^)
